...
if __name__ == "__main__":
    ...
    savehp.help = f"If using `gridsearch` method, save the optimum hyperparameters to `{os.path.basename(cfg.bdtComb_opts['optHyperParamsFile'])}`, default = False"
    loadhp.help = f"If using `fixed-hyperparams` method, use `{os.path.basename(cfg.bdtComb_opts['optHyperParamsFile'])}`, default = False"
    savemo.help = f"Save model to `{os.path.basename(cfg.bdtComb_opts['jsonPath'])}` and `{os.path.basename(cfg.bdtComb_opts['mvaPath'])}`, default = False"
    ...
    if args.save_model:
        ...
        print(f"{15*' '}1. {cfg.bdtComb_opts['jsonPath']}")
        print(f"{15*' '}2. {cfg.bdtComb_opts['mvaPath']}")
